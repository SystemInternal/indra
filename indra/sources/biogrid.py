...
class BiogridProcessor(object):
    ...
    def __init__(self, biogrid_file=None, physical_only=True):
        ...
        # If no file is provided, download from web
            ...
            logger.info(
                "No data file specified, downloading from BioGrid "
        ...
        for row in tqdm.tqdm(rows, desc="Processing BioGRID rows"):
            ...
            filt_row = [None if item == "-" else item for item in row][: len(columns)]
            ...
            agent_a = self._make_agent(
                bg_row.symbol_a, bg_row.entrez_a, bg_row.swissprot_a, bg_row.trembl_a
            )
            agent_b = self._make_agent(
                bg_row.symbol_b, bg_row.entrez_b, bg_row.swissprot_b, bg_row.trembl_b
            )
...
def _download_biogrid_data(url):
    ...
    if res.status_code != 200:
        raise Exception(
            "Unable to download Biogrid data: status code %s" % res.status_code
        )
    ...
    if len(zip_info_list) != 1:
        raise Exception(
            "There should be exactly zipfile in BioGrid zip "
