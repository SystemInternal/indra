...
class ReachProcessor(object):
    ...
    def get_agents_from_entities(self):
        ...
        agents = [self._get_agent_from_entity(e["frame_id"])[0] for e in entities]
                ...
                for reg in reg_res:
                    ...
                    if reg_subtype == "negative-regulation":
                        ...
                        if not modification_type:
                            logger.warning(
                                "Unhandled modification type: %s" % modification_type
                            )
                            continue
    ...
    def get_regulate_amounts(self):
        ...
        for r in all_res:
            ...
            theme_agent, theme_coords = self._get_agent_from_entity(theme)
            qstr = (
                "$.events.frames[((@.type is 'regulation') or "
    ...
    def get_activation(self):
        ...
        for r in res:
            ...
            for a in args:
                if self._get_arg_type(a) == "controller":
                    controller_agent, controller_coords = self._get_controller_agent(a)
                if self._get_arg_type(a) == "controlled":
                    controlled = a["arg"]
            controlled_agent, controlled_coords = self._get_agent_from_entity(
                controlled
            )
            if controller_agent is None or controlled_agent is None:
                continue
            annotations["agents"]["coords"] = [controller_coords, controlled_coords]
            ...
            st = stmt_cls(
                controller_agent, controlled_agent, **stmt_kwargs, evidence=ev
            )
            self.statements.append(st)
        ...
        for r in res:
            ...
            for a in args:
                ...
                if self._get_arg_type(a) == "substrate":
                    substrate_agent, substrate_coords = self._get_agent_from_entity(
                        a["arg"]
                    )
                if self._get_arg_type(a) == "product":
                    product_agent, product_coords = self._get_agent_from_entity(
                        a["arg"]
                    )
            if not all({controller_agent, substrate_agent, product_agent}):
                continue
            annotations["agents"]["coords"] = [
                controller_coords,
                substrate_coords,
                product_coords,
            ]
            st = Conversion(
                controller_agent, [substrate_agent], [product_agent], evidence=[ev]
            )
            self.statements.append(st)
    ...
    def _get_agent_from_entity(self, entity_id, sentence_coords=True):
        ...
        coords = self._get_entity_coordinates(
            entity_term, sentence_coords=sentence_coords
        )
    ...
    def _get_db_refs(entity_term, ontology, organism_priority=None):
        ...
        if db_refs.get("UP"):
            if organism_priority:
                ...
                unique_altxrefs = set(
                    (axr["namespace"], axr["id"])
                    for axr in entity_term.get("alt-xrefs", [])
                )
                # This returns a single prioritized UniProt ID or None
                prioritized_id = prioritize_organism_grounding(
                    db_refs["UP"], unique_altxrefs, organism_priority
                )
    ...
    def _get_annot_context(self, frame_term):
        annotations = {"found_by": frame_term["found_by"], "agents": {}}
        try:
    ...
    def _get_controller_agent(self, arg):
        ...
        # Or the controller is a complex
            ...
            controller_agent, coords = self._get_agent_from_entity(controllers[0])
            bound_agents = [self._get_agent_from_entity(c)[0] for c in controllers[1:]]
            bound_conditions = [BoundCondition(ba, True) for ba in bound_agents]
    ...
    def _parse_mutation(s):
        ...
        if m is not None:
            ...
            return mut
        elif s.lower() in ("mutation", "mutations", "mutant", "mutants", "mutational"):
...
_site_pattern2 = (
    "("
    + "|".join([v["short_name"].upper() for v in amino_acids.values()])
    + ")[- ]?([0-9]+)$"
)
_site_pattern3 = (
    "("
    + "|".join([v["indra_name"].upper() for v in amino_acids.values()])
    + ")[^0-9]*([0-9]+)$"
)
...
def _read_famplex_map():
    fname = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "../../resources/famplex_map.tsv"
    )
    famplex_map = {}
...
def _read_reach_rule_regexps():
    ...
    """
    reach_rule_filename = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "reach_rule_regexps.txt"
    )
